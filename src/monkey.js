...
(function (scope) {
  ...
  store.ready().then(() => {
    ...
    var $DEMO_MONKEY = new Monkey(store.getState().configurations, scope, settings.isFeatureEnabled('undo'), settings.monkeyInterval, settings.isFeatureEnabled('experimantal_withTemplateEngine'), urlManager, settings.isDebugEnabled())
    ...
    function restart() {
      ...
      // Update settings
      var settings = new Settings(store.getState().settings)
      var newMonkey = new Monkey(store.getState().configurations, scope, settings.isFeatureEnabled('undo'), settings.monkeyInterval, settings.isFeatureEnabled('experimantal_withTemplateEngine'), urlManager, settings.isDebugEnabled())
      $DEMO_MONKEY.stop()
